...
class AlbumRepository(Repository):
    ...
    Albums can contain any type of items via the album_items junction table.
    '''
    ...
    def create(
        ...
        self._execute(
            '''INSERT INTO albums
               (id, name, folder_id, user_id, cover_item_id, created_at)
               VALUES (?, ?, ?, ?, ?, ?)''',
            (album_id, name, folder_id, user_id, cover_item_id, datetime.now()),
        )
        self._commit()
        return album_id
