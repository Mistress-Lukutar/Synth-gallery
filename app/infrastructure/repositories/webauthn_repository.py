...
class AsyncWebAuthnRepository(AsyncRepository):
    """Async repository for WebAuthn credential management.
    
    Examples:
        >>> async def add_credential():
        ...     async with get_async_db() as conn:
        ...         repo = AsyncWebAuthnRepository(conn)
        ...         return await repo.create(1, b'...', b'...', "YubiKey")
    """
    ...
    async def create(
        ...
        """Add a new WebAuthn credential asynchronously."""
    ...
    async def get_for_user(self, user_id: int) -> list[dict]:
        """Get all WebAuthn credentials for a user asynchronously."""
    ...
    async def get_by_credential_id(self, credential_id: bytes) -> dict | None:
        """Get a WebAuthn credential by its credential_id asynchronously."""
    ...
    async def get_credential_ids_for_user(self, user_id: int) -> list[bytes]:
        """Get all credential IDs for a user asynchronously."""
    ...
    async def get_all_credential_ids(self) -> list[bytes]:
        """Get all credential IDs in the system asynchronously."""
        rows = await self._fetchall(
            "SELECT credential_id FROM webauthn_credentials"
        )
        return [row["credential_id"] for row in rows]
    
    async def update_sign_count(self, credential_id: bytes, new_sign_count: int) -> bool:
        """Update the sign count for a credential asynchronously."""
        cursor = await self._execute(
            "UPDATE webauthn_credentials SET sign_count = ? WHERE credential_id = ?",
            (new_sign_count, credential_id)
        )
    ...
    async def delete(self, credential_db_id: int, user_id: int) -> bool:
        """Delete a WebAuthn credential asynchronously."""
        cursor = await self._execute(
            "DELETE FROM webauthn_credentials WHERE id = ? AND user_id = ?",
            (credential_db_id, user_id)
        )
    ...
    async def rename(self, credential_db_id: int, user_id: int, new_name: str) -> bool:
        """Rename a WebAuthn credential asynchronously."""
        cursor = await self._execute(
            "UPDATE webauthn_credentials SET name = ? WHERE id = ? AND user_id = ?",
            (new_name, credential_db_id, user_id)
        )
    ...
    async def user_has_credentials(self, user_id: int) -> bool:
        """Check if user has any registered WebAuthn credentials asynchronously."""
    ...
    async def count_for_user(self, user_id: int) -> int:
        """Count credentials for a user asynchronously."""
        result = await self._fetchone(
            "SELECT COUNT(*) as count FROM webauthn_credentials WHERE user_id = ?",
            (user_id,)
        )
