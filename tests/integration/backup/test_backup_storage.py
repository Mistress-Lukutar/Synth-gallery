...
def _backup_env(env: dict) -> ExitStack:
    """Patch the backup module to operate inside ``env``'s temp directory.

    Returns an un-entered ExitStack; use ``with _backup_env(env):``.
    """
    stack = ExitStack()
    stack.enter_context(
        patch("app.infrastructure.services.backup.BASE_DIR", env["tmp_path"])
    )
    stack.enter_context(
        patch("app.infrastructure.services.backup.BACKUP_PATH", env["backup_dir"])
    )
    stack.enter_context(
        patch(
            "app.infrastructure.services.backup.get_storage",
            return_value=env["storage"],
        )
    )
    return stack
